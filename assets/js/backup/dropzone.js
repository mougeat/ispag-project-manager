...
function showConfirmationModal(datas_to_confirm, existing_datas) {
    ...
    console.log('showConfirmationModal', datas_to_confirm,  existing_datas);
    // 1. EXTRACTION DE LA LISTE DES CHAMPS
    let fields_list = [];
    if (Array.isArray(datas_to_confirm) && datas_to_confirm[0] && datas_to_confirm[0].fields) {
        fields_list = datas_to_confirm[0].fields;
    } else {
        fields_list = datas_to_confirm; 
    }

    if (!Array.isArray(fields_list) || fields_list.length === 0) return;

    const updateExistingValues = (index) => {
        if (existingDataArray.length === 0) return;
        const dataToDisplay = existingDataArray[index];
    ...
    html += '<thead><tr><th></th><th>Champ</th><th>Actuel</th><th>Nouveau</th></tr></thead><tbody>';

    // 2. UTILISATION DE fields_list (AU LIEU DE datas_to_confirm)
    fields_list.forEach(item => {
        // Sécurité pour la clé
        const safeKey = item.key || 'unknown';
        const label = safeKey.replace(/_/g, ' ');

        // Sécurité pour les valeurs (si item.new est un objet technique)
        ...
        html += `<tr data-key="${safeKey}">
            <td><input type="checkbox" name="confirm_field[]" value="${safeKey}" ${safeKey !== 'Id' ? '' : 'checked'}></td>
            <td><strong>${label}</strong></td>
            <td class="existing-value-cell">${item.existing ?? '---'}</td>
            <td class="new-value-cell">${newValue ?? '---'}</td>
        </tr>`;
    });
    ...
    // ... (Le reste des événements reste identique)
    ...
    jQuery('#confirmButton').off('click').on('click', function() {
        const dataToUpdate = {};
        jQuery('#confirmationForm input:checked').each(function() {
            const key = jQuery(this).val();
            dataToUpdate[key] = jQuery(this).closest('tr').find('.new-value-cell').text();
        });
