/**
 * ISPAG DXF ENGINE
 * Version 3.4.2 - FIX LABELS CHEVAUCHEMENT + BASE 3.4.1
 */
const IspagDxfEngine = {
    ...
    generateEntities: function(specs) {
        ...
        if (specs.piquages_techniques && Array.isArray(specs.piquages_techniques)) {
            // Dictionnaire pour suivre le nombre de piquages par angle (pour le décalage)
            const angleRegistry = {};
            ...
            specs.piquages_techniques.forEach((p, index) => {
                ...
                // --- VUE DESSUS (Label avec décalage anti-chevauchement) ---
                if (angleRegistry[angleDeg] === undefined) angleRegistry[angleDeg] = 0;
                // On augmente la distance pour chaque nouveau piquage sur le même angle
                const labelDist = R + 100 + (angleRegistry[angleDeg] * 120); 
                angleRegistry[angleDeg]++;

                const xTop = vdx + labelDist * sinA;
                const yTop = vdy + labelDist * cosA;
                
                // Trait de rappel vers le piquage
                entities.push(this.createLine(vdx + R * sinA, vdy + R * cosA, xTop, yTop, "PIQUAGES"));
                // Positionnement du texte Px
                entities.push(this.createMText(xTop + (20 * sinA), yTop + (20 * cosA), `P${index}`, 0, "PIQUAGES"));
            });
        }
